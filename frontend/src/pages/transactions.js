...
export default function Transactions() {
  ...
  const columns = [
    ...
    },
    {
      ...
      dataIndex: 'createdAt',
      key: 'createdAt',
      render: (date) => <span style={{ color: '#6B7280', fontSize: '0.9rem' }}>{dayjs(parseInt(date) || date).format('MMM D, YYYY h:mm A')}</span>
    },
    {
      title: 'Status',
      dataIndex: 'status',
  ...
  return (
    <div style={{ width: '100%', display: 'flex', flexDirection: 'column', gap: '32px' }}>
      ...
      <div style={{ padding: '40px', background: 'linear-gradient(135deg, #1B2A4E 0%, #312E81 50%, #4338CA 100%)', borderRadius: '32px', color: 'white', boxShadow: '0 20px 40px rgba(49, 46, 129, 0.2)' }}>
      ...
      <div style={{ background: '#FFF', borderRadius: '24px', boxShadow: '0 4px 20px rgba(0,0,0,0.02)', padding: '24px', overflowX: 'auto' }}>
        <Table 
          columns={columns} 
